...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if (!$CIN){
        ...
        $errors[]='mote de pass est obligatoire';
    }if (!$permis1){
        $errors[]='numméro de permis est obligatoire';
    }if (!$permis2){
        $errors[]='numméro de permis est obligatoire';
    }
    ...
    if (empty($errors)) {
        ...
        $statement = $pdo->prepare("INSERT INTO projet_location(cin,prenom,nom,email,mot_pass,numero) VALUES (:CIN, :Nom, :Prenom, :email, :password, :permis)");
        $statement->bindValue(':CIN', $CIN);
        $statement->bindValue(':Nom', $Nom);
        $statement->bindValue(':Prenom', $Prenom);
        $statement->bindValue(':email', $email);
        $statement->bindValue(':password', $password);
        $permis=$permis1 + $permis2;
        $statement->bindValue(':permis', $permis);
        $statement->execute();
    }
    else 
      echo "<h3 style='color:red;'>! votre données n'a pas enregistré car il ' ya un champ vide </h3><br><br>";
}
?>
<!DOCTYPE html>
<html>
...
<body>
   ...
   <div class="container">
       <div class="main">
           <div class="rows">
               ...
               <div class="col-md-8 mt-1 right-side">
                   <div class="card mb-3 content">
                       <h1 class="m-3 pt-3" style="color: green; font-weight: bold;">Merci,votre page a été enregistrées avec succès!</h1>
           ...
           <button style="background-color: #f44336; width: 80px; height: 30px; border-radius: 5px;">
            <a href="c.html" style="text-decoration: none; color:white; font-size: 20px;">Retour</a></button>
       </div>
   </div>
    <footer id="contact">
          <div class="bas">
